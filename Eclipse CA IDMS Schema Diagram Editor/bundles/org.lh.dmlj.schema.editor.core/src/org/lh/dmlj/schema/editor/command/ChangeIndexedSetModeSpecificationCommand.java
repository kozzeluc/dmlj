...
public class ChangeIndexedSetModeSpecificationCommand extends ModelChangeBasicCommand {
	...
	private final String newSymbolicIndexName;
	private final Short newKeyCount;
	private final Short newDisplacementPages;

	public ChangeIndexedSetModeSpecificationCommand(Set set, String symbolicIndexName, Short keyCount, Short displacementPages) {
	...
	public void execute() {
		...
		Assert.isTrue(newSymbolicIndexName != null || newKeyCount != null, "neither symbolic index name nor key count specified");
		Assert.isTrue(newSymbolicIndexName == null || newKeyCount == null, "symbolic index name AND key count specified");
		Assert.isTrue(!(newKeyCount == null && newDisplacementPages != null), "key count NOT specified");
		
		if (newSymbolicIndexName != null) {
			var validationResult = NamingConventions.validate(newSymbolicIndexName, NamingConventions.Type.SYMBOLIC_INDEX_NAME);
			Assert.isTrue(validationResult.getStatus() == OK, "symbolicIndexName is invalid (" + validationResult.getMessage() +	")");
		}
		Assert.isTrue(newKeyCount == null || newKeyCount.shortValue() >= 3 && newKeyCount.shortValue() <= 8180, 
