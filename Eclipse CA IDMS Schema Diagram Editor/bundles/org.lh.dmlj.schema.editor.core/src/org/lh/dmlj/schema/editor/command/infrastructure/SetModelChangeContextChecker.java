...
public final class SetModelChangeContextChecker extends ModelChangeContextChecker<Set> {

	@Override
	public boolean appliesTo(ModelChangeContext context, Set set) {
		checkAppliesTo(context, set);
		var contextData = context.getContextData();
		return contextData.size() == 1 &&
				set.getName().equals(contextData.get(ContextDataKeys.SET_NAME)) || contextData.size() == 2 &&
