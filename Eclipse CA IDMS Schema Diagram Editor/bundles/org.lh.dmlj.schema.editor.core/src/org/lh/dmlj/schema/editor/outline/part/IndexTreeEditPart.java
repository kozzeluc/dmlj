...
public class IndexTreeEditPart extends AbstractSchemaTreeEditPart<SystemOwner> {
	...
	public List<?> getModelChildren() {
		return List.of(getModel().getSet().getMembers().get(0).getRecord());
	}
	...
	protected void registerModel() {
		// different edit parts exist for the same index; make sure that selecting an index in the SchemaEditor
		// yields the outline view's top level index to become the current selection
		...
		if (parentModelObject instanceof Schema) {
			...
			// make sure that index edit parts that are not at the top level will never be found by their model
