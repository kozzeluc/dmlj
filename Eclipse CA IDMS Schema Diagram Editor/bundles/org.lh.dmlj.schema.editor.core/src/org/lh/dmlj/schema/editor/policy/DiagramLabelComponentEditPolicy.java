...
public class DiagramLabelComponentEditPolicy extends ComponentEditPolicy {
	...
	protected Command createDeleteCommand(GroupRequest deleteRequest) {
		...
		if (editParts.size() > 1) {
			// if multiple items are selected in the diagram don't allow to delete the diagram label by returning null
			return null;
		}
		Assert.isTrue(editParts.size() == 1, "exactly 1 diagram label expected");
		Assert.isTrue(editParts.get(0) instanceof DiagramLabelEditPart || editParts.get(0) instanceof DiagramLabelTreeEditPart,
				"expected a diagram label");
		
		DiagramLabel diagramLabel;
		if (editParts.get(0) instanceof DiagramLabelEditPart diagramLabelEditPart) {
			diagramLabel = diagramLabelEditPart.getModel();
		} else {
			DiagramLabelTreeEditPart editPart = (DiagramLabelTreeEditPart) editParts.get(0); 
			diagramLabel = editPart.getModel();
		}
		
		var context = new ModelChangeContext(ModelChangeType.DELETE_DIAGRAM_LABEL);
		var command = new DeleteDiagramLabelCommand(diagramLabel.getDiagramData().getSchema());
