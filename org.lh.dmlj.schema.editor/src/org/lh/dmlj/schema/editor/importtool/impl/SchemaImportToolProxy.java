...
public final class SchemaImportToolProxy {
	...
	private void handleRecord(Object recordContext) {						
		...
		// get the record's storage mode
		StorageMode storageMode = 
			dataCollector.getStorageMode(recordContext);
		...
		// is created later on
		SchemaRecord record = 
			modelFactory.createRecord(recordName, recordId, storageMode, 
		...
		if (isIdmsntwk() && 
			(record.getName().equals("OOAK-012")) &&
			 tool.isOptionCompleteOoak_012() ||			 
			 record.getName().equals("LOOAK-155") &&
			 tool.isOptionCompleteLooak_155()) {
